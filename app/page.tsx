...
const FAQS = [
  ...
  { q: 'What is a programmatic SEO system?',
     a: 'A system that deploys thousands of targeted pages at scale — each engineered to rank for specific high-intent search queries. Instead of one or two pages, we build hundreds or thousands, capturing demand across an entire city or region.' },
  ...
  { q: 'How do leads get delivered?',
     a: 'Captured directly from the system and delivered to your inbox and phone in real time. No shared lead pools. No middlemen. Every lead is exclusively yours.' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: ''DM Sans', sans-serif', color: '#111111' }}>
      ...
      <section style={{
        ...
        <div style={{ ...C.wrap, position: 'relative', zIndex: 1, padding: '6rem 1.5rem' }}>
          <div style={{ maxWidth: '720px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '2rem' }}>
              <div style={{...C.rule}} />
              <span style={{...C.label}}>Lead Generation Systems</span>
            </div>
            <h1 style={{ ...C.serif, fontSize: 'clamp(2.4rem, 6vw, 5.5rem)', lineHeight: 1.05, marginBottom: '2rem', color: '#111111' }}>
            ...
            <p style={{ ...C.mono, fontSize: '0.7rem', color: '#999', letterSpacing: '0.15em', marginBottom: '2rem' }}>
              Limited availability per market
            </p>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <div style={{ height: '1px', width: '32px', background: '#B8860B' }} />
              <p style={{ ...C.mono, fontSize: '0.7rem', color: '#888', letterSpacing: '0.18em', textTransform: 'uppercase' }}>
      ...
      <section style={{ background: '#fff', padding: '5rem 0', borderTop: '1px solid #E5E5E5' }}>
        <div style={{...C.wrap}}>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-end', gap: '1.5rem', marginBottom: '4rem' }}>
            <div>
              <span style={{...C.label, display: 'block', marginBottom: '0.75rem'}}>Verified Results</span>
              <h2 style={{ ...C.serif, fontSize: 'clamp(2rem, 4vw, 3rem)', color: '#111111' }}>The Numbers Don't Lie</h2>
            </div>
          ...
          <div style={{ borderLeft: `3px solid #B8860B`, paddingLeft: '1.5rem', marginTop: '4rem', maxWidth: '680px' }}>
            <p style={{ ...C.serif, fontSize: 'clamp(1.1rem, 2vw, 1.4rem)', lineHeight: 1.6, fontStyle: 'italic', color: '#111111' }}>
            ...
            <p style={{ ...C.mono, fontSize: '0.7rem', color: '#B8860B', letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem', alignItems: 'center' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem' }}>
                <div style={{...C.rule}} />
                <span style={{...C.label}}>Case Study</span>
              </div>
              <h2 style={{ ...C.serif, fontSize: 'clamp(1.8rem, 3.5vw, 3rem)', lineHeight: 1.15, marginBottom: '1.5rem', color: '#111111' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', borderTop: '1px solid #E5E5E5', paddingTop: '2rem', marginBottom: '2.5rem' }}>
                {[{ n: '30K+', l: 'Pages Built' }, { n: '22K+', l: 'Impressions / 7 Days' }, { n: '$1K', l: 'Per Lead Value' }].map(x => (
                  <div key={x.l}>
                    <div style={{ ...C.mono, fontSize: '1.5rem', fontWeight: 700, color: '#111111', marginBottom: '4px' }}>{x.n}</div>
      ...
      <section style={{ background: '#fff', padding: '5rem 0', borderTop: '1px solid #E5E5E5' }}>
        <div style={{...C.wrap}}>
          <div style={{ marginBottom: '4rem' }}>
            <span style={{...C.label, display: 'block', marginBottom: '0.75rem'}}>Our Systems</span>
            <h2 style={{ ...C.serif, fontSize: 'clamp(1.8rem, 3.5vw, 3rem)', color: '#111111', maxWidth: '480px' }}>What We Build</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', borderTop: '1px solid #E5E5E5' }}>
            {SERVICES.map((s, i) => (
              <div key={s.num} style={{ borderRight: i < 2 ? '1px solid #E5E5E5' : 'none', padding: '2.5rem 2rem 2.5rem', paddingLeft: i === 0 ? 0 : '2rem' }}>
                <div style={{ ...C.mono, fontSize: '0.7rem', color: '#B8860B', letterSpacing: '0.18em', marginBottom: '1.5rem' }}>{s.num}</div>
                <h3 style={{ ...C.serif, fontSize: '1.5rem', lineHeight: 1.2, color: '#111111', marginBottom: '1rem' }}>{s.title}</h3>
                <p style={{ fontSize: '0.875rem', color: '#666', lineHeight: 1.7, marginBottom: '1.5rem' }}>{s.desc}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {s.tags.map(t => (
                    <span key={t} style={{ ...C.mono, fontSize: '0.6rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#B8860B', border: `1px solid rgba(184,134,11,0.3)`, padding: '3px 8px' }}>{t}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem', alignItems: 'center' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '2rem' }}>
                ...
                <span style={{ ...C.mono, fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#B8860B' }}>Our Model</span>
              </div>
              <h2 style={{ ...C.serif, fontSize: 'clamp(2rem, 4vw, 3.5rem)', lineHeight: 1.1, color: '#fff', marginBottom: '2rem' }}>
            ...
            <div style={{ border: '1px solid #2A2A2A' }}>
              {FEATURES.map((f, i) => (
                <div key={f.label} style={{ display: 'flex', gap: '1rem', padding: '1.25rem', borderBottom: i < FEATURES.length - 1 ? '1px solid #2A2A2A' : 'none' }}>
                  <div style={{ ...C.mono, fontSize: '0.7rem', color: '#B8860B', flexShrink: 0, marginTop: '2px' }}>{String(i+1).padStart(2,'0')}</div>
                  <div>
      ...
      <section style={{ background: '#fff', padding: '5rem 0', borderTop: '1px solid #E5E5E5' }}>
        <div style={{...C.wrap}}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            <span style={{...C.label, display: 'block', marginBottom: '0.75rem'}}>How It Works</span>
            <h2 style={{ ...C.serif, fontSize: 'clamp(1.8rem, 3.5vw, 3rem)', color: '#111111' }}>The System Architecture</h2>
          </div>
      ...
      <section style={{ background: '#fff', padding: '5rem 0', borderTop: '1px solid #E5E5E5' }}>
        <div style={{...C.wrap}}>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-end', gap: '1.5rem', marginBottom: '3rem' }}>
            <div>
              <span style={{...C.label, display: 'block', marginBottom: '0.75rem'}}>Our Reach</span>
              <h2 style={{ ...C.serif, fontSize: 'clamp(1.8rem, 3.5vw, 3rem)', color: '#111111' }}>Markets We Deploy Systems In</h2>
            </div>
            <p style={{ fontSize: '0.875rem', color: '#888', maxWidth: '280px', lineHeight: 1.6 }}>
              One methodology. Multiple markets.
            </p>
          </div>
          <div style={{ borderTop: '1px solid #E5E5E5' }}>
            {MARKETS.map(m => (
              <div key={m.market} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 2fr', gap: '1.5rem', padding: '2rem 0', borderBottom: '1px solid #F0F0F0', alignItems: 'baseline' }}>
                <h3 style={{ ...C.serif, fontSize: '1.4rem', color: '#111111' }}>{m.market}</h3>
                <p style={{ fontSize: '0.875rem', color: '#666', lineHeight: 1.6 }}>{m.desc}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {m.examples.map(e => (
                    <span key={e} style={{ ...C.mono, fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#888', border: '1px solid #E5E5E5', padding: '3px 8px' }}>{e}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      <section style={{ background: '#F8F7F4', padding: '5rem 0', borderTop: '1px solid #E5E5E5' }}>
        <div style={{...C.wrap}}>
          <div style={{ marginBottom: '3rem' }}>
            <span style={{...C.label, display: 'block', marginBottom: '0.75rem'}}>Common Questions</span>
            <h2 style={{ ...C.serif, fontSize: 'clamp(1.8rem, 3.5vw, 3rem)', color: '#111111' }}>What You Should Know</h2>
          </div>
            ...
            {FAQS.map((f, i) => (
              <details key={i} style={{ borderBottom: '1px solid #E5E5E5' }}>
                <summary style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem 0', cursor: 'pointer', listStyle: 'none', gap: '1rem' }}>
                  <span style={{ ...C.serif, fontSize: '1.05rem', color: '#111111', fontWeight: 600 }}>{f.q}</span>
                  <span style={{ ...C.mono, color: '#B8860B', fontSize: '1.2rem', flexShrink: 0 }}>+</span>
                </summary>
      ...
      <section style={{ background: '#fff', padding: '6rem 0', borderTop: '1px solid #E5E5E5' }}>
        <div style={{...C.wrap}}>
          <div style={{ maxWidth: '720px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '2rem' }}>
              <div style={{...C.rule}} />
              <span style={{...C.label}}>Ready to Scale?</span>
            </div>
            <h2 style={{ ...C.serif, fontSize: 'clamp(2.2rem, 5vw, 4.5rem)', lineHeight: 1.05, color: '#111111', marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <a href="tel:3108461658" className="btn-ghost" style={{ ...C.mono, fontSize: '0.8rem', letterSpacing: '0.1em' }}>(310) 846-1658</a>
            </div>
          </div>
        </div>
      </section>
